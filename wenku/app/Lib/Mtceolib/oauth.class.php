...
class oauth {
    ...
    public function __construct($type) {
        ...
        //导入接口文件
        include_once LIB_PATH . 'Mtceolib/oauth/' . $type . '/' . $type . '.php';
    ...
    public function callbackLogin($request_args) {
        ...
        if ($bind_user) {
            ...
            return U('ucenter/index');
        } else {
            //处理用户名
            if (M('user')->where(array('username' => $user['keyname']))->count()) {
                $user['Mtceo_user_name'] = $user['keyname'] . '_' . mt_rand(99, 9999);
            } else {
                $user['Mtceo_user_name'] = $user['keyname'];
            }
            $user['Mtceo_user_name'] = urlencode($user['Mtceo_user_name']);
            $user['keyname'] = urlencode($user['keyname']);
            if ($user['keyavatar_big']) {
                //下载原始头像到本地临时储存  用日期文件夹分类  方便清理
                $user['temp_avatar'] = '';
                $avatar_temp_root = C('Mtceo_attach_path') . 'avatar/';
    ...
    private function _oauth_visitor() {
        include_once (MTCEO_PATH . 'app/Lib/Mtceolib/user_visitor.class.php');
