	 ...
	 if(in_array($ext, $extarr)){
	    ...
	    $filepath='./' . C('mtceo_attach_path').'docswf/data/'.$filename.'/';
	 	if(!file_exists('./' . C('mtceo_attach_path')."docswf/data/".$filename.".php")){ 
	 		...
	 		//如果没有该文件则显示未生成预览
	 		$ipstr="<div class=\"stl_01 mod reader-page complex reader-page-\"><div class=\"nopreview\">该文档尚未生成预览</div></div>";
	 	}	
	 }//如果不存在缓存文件的判断结束
	 else{
...
function getitemraty($id,$typeid){//评分
...
function getflink($id='1',$img=false){
	...
	$map['cate_id']=$id;
	$map['status']=1;
	if($img){$map['img']=array('neq','');}else{$map['img']='';}
...
function docimg($model,$id){
	...
	return 'http://www.mtceo.net/data/upload/docswf/preview/'.$info['viewurl'].'0001.jpg';	
		
		
		
	}else{
		
		if(file_exists(C('mtceo_attach_path') . 'docswf/preview/'.$info['filename'].'0001.jpg')){
			
			
			return __ROOT__ . '/' . C('mtceo_attach_path') . 'docswf/preview/'.$info['filename'].'0001.jpg';
			
		}else{
			
			return __ROOT__ . '/' . C('mtceo_attach_path') . 'docswf/preview/default.png';
		}
