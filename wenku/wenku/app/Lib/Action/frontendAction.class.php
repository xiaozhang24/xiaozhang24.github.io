...
class frontendAction extends baseAction {
    ...
    public function _initialize() {
        ...
        if (!C('mtceo_site_status')) {
            header('Content-Type:text/html; charset=utf-8');
            exit(C('mtceo_closed_reason'));
        }
    ...
    */
    ...
    protected function _config_seo($seo_info = array(), $data = array()) {
        $page_seo = array(
            'title' => C('mtceo_site_title'),
            'keywords' => C('mtceo_site_keyword'),
            'description' => C('mtceo_site_description')
        );
        $page_seo = array_merge($page_seo, $seo_info);
        //开始替换
        $searchs = array('{site_name}', '{site_title}', '{site_keywords}', '{site_description}');
        $replaces = array(C('mtceo_site_name'), C('mtceo_site_title'), C('mtceo_site_keyword'), C('mtceo_site_description'));
    ...
    */
    protected function _user_server() {
        $passport = new passport(C('mtceo_integrate_code'));
        return $passport;
    }
...
/**
	...
	public function index() {
		...
		if($this->_name=='user_role'){$this->_list($mod, $map,'score','asc');}
	 ...
	 */
	protected function _list($list_relation,$model, $map = array(), $sort_by='', $order_by='', $field_list='*', $pagesize=10)
	{
		//排序
		$data=array();
		$mod_pk = $model->getPk();
