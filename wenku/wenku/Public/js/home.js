...
;$(function($){
	...
	$('.J_shownoform').live('click', function(){
		...
		if(duid == ''){
			$.mtceo.tip({content:'请先登陆或者注册才能下载哦！！',icon:'alert'});
			return false;
		}
        ...
        $('.J_cate_select').die('change').live('change', function(){
            ...
            if(_pid != ''){
                $.getJSON(uri, {id:_pid}, function(result){
                    if(result.status == '1'){
                        var _childs = $('<select size=10 class="J_cate_select mr10" data-pid="'+_pid+'"><option value="">--'+settings.top_option+'--</option></select>')
                        for(var i=0; i<result.data.length; i++){
