...
<div class="ucright">
<div class="menuname">
<ul><li class="cur"><a href="#">我的消息</a></li>
...
<div class="submenu">
<ul>
<li><a href="<?php echo U('ucenter/usermail');?>" class="cur">消息列表</a></li>
...
<?php if(is_array($data['list'])): $i = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "暂无消息" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><li <?php if($vo['fromid'] != $uid): ?>class="maillist"<?php endif; ?>>
<?php if($vo['fromid'] == $uid): ?><input type="checkbox" class="check J_checkitem" value="<?php echo ($vo["id"]); ?>"/><?php endif; ?>

<?php if($vo["is_sys"] == 1): ?><font color=red>[公共<?php else: ?><font color=blue>[私信<?php endif; ?>]</font>
<a href="<?php echo U('ucenter/mailmess',array('id'=>$vo['id']));?>"><?php echo ($vo["title"]); ?>

</a>
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;


<s class="pbclass" datauri="<?php echo U('ucenter/pbitem',array('id'=>$vo['id']));?>" onclick="pb_item(this)">屏蔽</s>
<?php if((($vo['new'] == 1) AND ($vo['toid'] == $uid) ) OR ($vo['renew'] == 1)): ?><s>(新)</s><?php endif; ?>

<div class="fr">发布时间：<?php echo (fdate($vo["add_time"])); ?></div>
</li><?php endforeach; endif; else: echo "暂无消息" ;endif; ?>
...
<input type="checkbox" name="checkall" class="check J_checkall">全选/取消</label>
<input type="button" class="clear wk-btn2 g-n-canbtns" data-tdtype="batch_action" data-acttype="ajax" data-uri="<?php echo U('ucenter/mail_del');?>" data-name="id"  data-msg="<?php echo L('confirm_delete');?>" value="删   除" />
